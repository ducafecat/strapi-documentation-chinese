...
- 点击 `Users`, in the left hand menu, and then click `Add User`:
  ...
  - 点击 the `AWS Management Console Access sign-in link`. This will log you out of `Administrator`.
...
#### 3. 点击 the blue `Launch Instance` button
...
- Ensure `General purpose` + `t2.small` is `checked`.
  ...
  :::
- 点击 grey `Next: Configure Instance Details` and `Next: Add Storage`
...
- In the **Step 6: Configure Security Group**, configure the `security settings` as follows:
  ...
  - 点击 grey `Add rule` to add each of these rules:
...
- 点击 `1 Running Instance` and note the `IPv4 Public OP` address. E.g. `34.182.83.134`.
...
- You will need to access the `Settings` tab for your `Strapi Project Repository`:
  ...
  2. 点击 `Webhooks`, then click `Add Webhook`.
