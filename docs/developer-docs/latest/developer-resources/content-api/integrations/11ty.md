...
- 创建 a `./src/_data` folder containing a `categories.js` and a `restaurants.js` file. They will be used to fetch your content from Strapi.
- 创建 a `./src/_templates` folder containing a `default.liquid` file. It will be the default template of your project.
...
- 创建 a `./src/index.md`, `./src/restaurant.md` and a `./src/categorie.md` file. They will define how you'll present the data.
- 创建 a `.eleventy.js` file containing the following (make sure to prefix the file's name with the dot):
